...
function generatePassword() {
  ...
  //Password Length Prompt Storage
  var pwlength = parseInt(window.prompt("Please select a password length between 8 and 128 characters!"));
  //Password Length Acceptor, If not acceptor starts over
  if (pwlength > 7 && pwlength < 129 && Number.isInteger(pwlength)) {
    window.confirm("Are you sure you would like a password length of " + pwlength + "?");
  }else {
    window.alert("Your pasword length is not acceptable. Please try again!");
    return generatePassword();
  }

  //Confirm Prompt Testers
